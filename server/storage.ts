...
export class DatabaseStorage implements IStorage {
  ...
  async getDashboardStats(companyId?: number, branch?: string | null, user?: DashboardUserScope): Promise<any> {
    ...
    const [
      ...
      db.select({
        total: count(),
        completed: sql<number>`count(*) filter (where ${activities.status} = 'Selesai')`,
      }).from(activities).where(actConditions),
      db.select({
        total: count(),
        active: sql<number>`count(*) filter (where ${cases.status} != 'Closed')`,
        overdue: sql<number>`count(*) filter (where ${cases.status} != 'Closed' and ${cases.targetDate} < ${today})`,
        ...
        longWaiting: sql<number>`count(*) filter (where ${cases.status} != 'Closed' and ${cases.workflowStage} IN ('Proses Regulator', 'Settlement / Deadlock') and ${cases.dateReceived} < ${thirtyDaysAgoStr})`,
        highRisk: sql<number>`count(*) filter (where ${cases.riskLevel} = 'High' and ${cases.status} != 'Closed')`,
      }).from(cases).where(caseConditions),
      db.select({
        total: count(),
        pending: sql<number>`count(*) filter (where ${tasks.status} != 'Selesai')`,
        completed: sql<number>`count(*) filter (where ${tasks.status} = 'Selesai')`,
    ...
    return {
      totalActivities: Number(activityStats?.total) || 0,
      ...
      closedCases: Number(caseStats?.closed) || 0,
      overdueCases: Number(caseStats?.overdue) || 0,
      waitingCases: Number(caseStats?.waiting) || 0,
      longWaitingCases: Number(caseStats?.longWaiting) || 0,
      totalTasks: Number(taskStats?.total) || 0,
      pendingTasks: Number(taskStats?.pending) || 0,
      completedTasks: Number(taskStats?.completed) || 0,
      totalAnnouncements: Number(announcementStats?.total) || 0,
      highRiskCasesCount: Number(caseStats?.highRisk) || 0,
      recentActivities,
      recentCases,
      highRiskCases,
      casesByCompany,
  ...
  async calculateKpiForUser(userId: number, period: string): Promise<any> {
    ...
    const [
      ...
      }).from(users).where(and(sql`${users.role} IN ('du', 'dk', 'cbo', 'ceo', 'kepatuhan_cabang', 'apuppt')`, eq(users.isActive, true))),
      db.select({
        uid: users.id,
        contrib: sql<number>`(
  ...
  async calculateLiveKpi(userId: number): Promise<any> {
    ...
    const [
      ...
      db.select({
        ...
        sql`${users.role} IN ('du', 'dk', 'cbo', 'ceo', 'kepatuhan_cabang', 'apuppt')`,
